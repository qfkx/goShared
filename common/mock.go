...
func RandNumber(n int) string {
	a := "%0" + strconv.Itoa(n) + "v"
	return fmt.Sprintf(a, rand.New(rand.NewSource(time.Now().UnixNano())).Int31n(1000000))
}

// RandInt64 获取区间随机数
func RandInt64(min, max int64) int64 {
	return rand.New(rand.NewSource(time.Now().UnixNano())).Int63n(max-min+1) + min
}
...
func RandomTime() string {
	...
	var s2 int64
	s1 = 3600 * 24 * 180
	s2 = carbon.Now().ToTimestamp() - s1
	rand.Seed(time.Now().UnixNano())
	res := rand.Int63n(s1) + s2
	return carbon.CreateFromTimestamp(res).ToFormatString("Y年m月d日 H时i分")
}
...
func RandomTime2(n int64) time.Time {
	...
	rand.Seed(time.Now().UnixNano())
	res := rand.Int63n(s1) + s2
	return carbon.CreateFromTimestamp(res).Time
}

// GetYL 获取语录
func GetYL() string {
	resp, err := http.Get("http://api.btstu.cn/yan/api.php")
	if err != nil {
		fmt.Println(err)
	}
...
func GetToday() string {
	c := carbon.SetTimezone("Local")
	today := c.Now().Format("Y-m-d H:i:s")
	return today
}
...
func GetRandName() string {
	...
	rand.Seed(time.Now().UnixNano())
	l := rand.Intn(2)
	for i := 0; i <= l; i++ { //随机产生2位或者3位的名
		n = rand.Intn(firstNameLen - 1)
		v = firstName[n]
		first += fmt.Sprint(v)
	}
	last := fmt.Sprint(lastName[rand.Intn(lastNameLen-1)])
	full := fmt.Sprintf("%s%s", last, first)
	return full
}
